...
**The player is a Filter Set parameter, not a selection beside one.** Choosing a player patches `player_name` into the Filter Set in the address bar and nothing else, so a player change refetches under the filters already applied, and comparing two players under identical conditions is one action. Because the URL is the single source of truth, the applied-filter badges cannot drift from the request that produced the data underneath them.
