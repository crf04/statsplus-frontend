...
test('box combinations and per36 read the game line and the season total ratio', () => {
  ...
  expect(gameStat({ line }, 'FG2A')).toBe(10);
  expect(gameStat({ line }, 'FG2A/36')).toBe(15);
  ...
  expect(
    seasonStat(
      {
        seasonTotals: { ...line, field_goals_attempted: 40, threes_attempted: 15, minutes: 60 },
        seasonGames: 2,
      },
      'FG2A',
    ),
...
test('missing evidence and zero denominators are absent, while real zero production remains zero', () => {
  expect(gameStat({ line: { ...line, field_goals_attempted: undefined } }, 'FG2A')).toBeNull();
  expect(gameStat({ line: { ...line, threes_attempted: undefined } }, 'FG2A')).toBeNull();
  expect(
    gameStat({ line: { ...line, field_goals_attempted: 0, threes_attempted: 0 } }, 'FG2A'),
  ).toBe(0);
  expect(gameStat({ line: { ...line, minutes: 0 } }, 'PTS/36')).toBeNull();
  expect(gameStat({ line: { ...line, minutes: 0 } }, 'FG2A/36')).toBeNull();
...
test('season efficiency uses totals rather than an average of game percentages', () => {
  ...
  expect(
    seasonStat({ seasonTotals: { ...line, minutes: 0 }, seasonGames: 2 }, 'FG2A/36'),
  ).toBeNull();
});

test('FG2A per36 aggregate evidence uses total minutes for the player baseline', () => {
  const player = {
    seasonTotals: { ...line, field_goals_attempted: 40, threes_attempted: 15, minutes: 60 },
    seasonGames: 2,
  };
  const evidence = aggregateEvidence(
    { line: { ...line, field_goals_attempted: 10, threes_attempted: 4, minutes: 12 } },
    player,
    'FG2A/36',
  );
